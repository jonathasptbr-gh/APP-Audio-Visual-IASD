...
O espelho de pixels acrescentou três (v5.141): o **oráculo do
...
por construção, e ali essas APIs vêm `undefined`) e o caso do **papel espelho**
no `ponte.test.mjs` (o dreno deixa passar só `display-ready`, o
`BroadcastChannel.postMessage` é no-op — **e o par negativo com `role:'display'`**,
senão o dreno pode vazar para o telão de verdade e ninguém vê). O
...
**Versão atual: v5.141** (base web) · `SHELL_VERSION` **32**, e o bundle segue com
